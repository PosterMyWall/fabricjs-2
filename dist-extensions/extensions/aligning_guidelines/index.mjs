...
import { drawLine, drawX, drawPointList, drawVerticalLine, drawHorizontalLine } from './util/draw.mjs';
import { collectLine } from './util/collect-line.mjs';
import { getObjectsByTarget } from './util/get-objects-by-target.mjs';
import { getPointMap, getContraryMap } from './util/basic.mjs';

class AligningGuidelines {
  constructor(canvas) {
    let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    _defineProperty(this, "canvas", void 0);
    _defineProperty(this, "horizontalLines", new Set());
    _defineProperty(this, "verticalLines", new Set());
     ...
     */
    _defineProperty(this, "onlyDrawPoint", false);
    /** Alignment method is required when customizing. */
    _defineProperty(this, "contraryOriginMap", {
      tl: ['right', 'bottom'],
      tr: ['left', 'bottom'],
      br: ['left', 'top'],
      bl: ['right', 'top'],
    ...
    _defineProperty(this, "margin", 4);
    /** Aligning line dimensions */
    _defineProperty(this, "width", 1);
    /** Aligning line color */
    _defineProperty(this, "color", 'rgba(255,0,0,0.9)');
    /** Close Vertical line, default false. */
    _defineProperty(this, "closeVLine", false);
    /** Close horizontal line, default false. */
    _defineProperty(this, "closeHLine", false);
    this.canvas = canvas;
  ...
  scalingOrResizing(e) {
    ...
    if (target.flipX) {
      if (corner.includes('l')) corner = corner.replace('l', 'r');else if (corner.includes('r')) corner = corner.replace('r', 'l');
    }
    if (target.flipY) {
      if (corner.includes('t')) corner = corner.replace('t', 'b');else if (corner.includes('b')) corner = corner.replace('b', 't');
    }
